...
bool SharedMemoryManager::Initialize(const std::wstring& name, DWORD blockSize, DWORD blockCount) {
    if (isInitialized) {
        return true;
    }
...
bool SharedMemoryManager::CreateMapFile(DWORD size) {
    // 确保初始状态干净
    hMapFile = INVALID_HANDLE_VALUE;
    pMapView = nullptr;
...
bool SharedMemoryManager::OpenMapFile() {
    // 确保初始状态干净
    hMapFile = INVALID_HANDLE_VALUE;
    pMapView = nullptr;
    pHeader = nullptr;
    ...
    // 验证现有内存布局
    pHeader = static_cast<SharedMemoryHeader*>(pMapView);
    ...
    pRequestBlocks = reinterpret_cast<Block*>(basePtr + SHARED_MEMORY_HEADER_SIZE);
    pResponseBlocks = reinterpret_cast<Block*>(basePtr + SHARED_MEMORY_HEADER_SIZE + 
...
void SharedMemoryManager::InitializeLayout() {
    pHeader = static_cast<SharedMemoryHeader*>(pMapView);
    pRequestBlocks = reinterpret_cast<Block*>(reinterpret_cast<BYTE*>(pMapView) + SHARED_MEMORY_HEADER_SIZE);
    pResponseBlocks = reinterpret_cast<Block*>(reinterpret_cast<BYTE*>(pRequestBlocks) + (pHeader->blockCount * pHeader->blockSize));
...
DWORD SharedMemoryManager::CalculateTotalSize() const {
...
bool SharedMemoryManager::WriteRequest(const void* data, DWORD dataSize, DWORD commandId, DWORD dataType) {
    ...
    Block& block = pRequestBlocks[writeIndex];
    
    // 检查block大小是否足够
    if (sizeof(Block) + dataSize > pHeader->blockSize) {
        return false;
    }
...
bool SharedMemoryManager::WriteResponse(const void* data, DWORD dataSize, DWORD commandId, DWORD dataType) {
    if (!IsReady() || data == nullptr || dataSize == 0) {
        return false;
    }
    ...
    Block& block = pResponseBlocks[writeIndex];
...
bool SharedMemoryManager::ReadRequest(void* buffer, DWORD bufferSize, DWORD& outCommandId, DWORD& outDataType) {
    ...
    Block& block = pRequestBlocks[readIndex];
...
bool SharedMemoryManager::ReadResponse(void* buffer, DWORD bufferSize, DWORD& outCommandId, DWORD& outDataType) {
    if (!IsReady() || buffer == nullptr || bufferSize == 0) {
        return false;
    }
    ...
    Block& block = pResponseBlocks[readIndex];
...
DWORD SharedMemoryManager::GetShmSize() const {
    if (!IsReady()) {
        return 0;
    }
...
DWORD SharedMemoryManager::GetBlockCount() const {
    if (!IsReady()) {
        return 0;
    }
...
DWORD SharedMemoryManager::GetBlockSize() const {
    if (!IsReady()) {
        return 0;
    }
...
DWORD SharedMemoryManager::GetVersion() const {
    if (!IsReady()) {
        return 0;
    }
